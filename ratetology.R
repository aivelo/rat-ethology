...
deps = c("car", "ggplot2","lme4","dplyr","tidyr","psych","plyr", "corrr","multcomp");
...
drop1(cb3, test="Chisq")
summary(glht(cb3, mcp(age="Tukey")))


##removed graphs - stacked bar graphs with error bars for continuous behaviour
#
#
...
drop1(pb2, test="Chisq")
summary(glht(pb2, mcp(age="Tukey")))
...
drop1(pb3, test="Chisq")
summary(glht(pb3, mcp(age="Tukey")))
...
drop1(pb4, test="Chisq")
summary(glht(pb4, mcp(age="Tukey")))
...
anova(pb7)
summary(pb7)
with(summary(pb7), 1 - deviance/null.deviance)

drop1(pb7, test="Chisq")
...
preds_a <- predict(pb1,newdata=pd_a, type="response", se.fit=TRUE)
pd_a$fit <- preds_a$fit
pd_a$se = preds_a$se.fit

preds_b <- predict(pb1,newdata=pd_b, type="response", se.fit=TRUE)
pd_b$fit <- preds_b$fit
pd_b$se = preds_b$se.fit

preds_juv <- predict(pb1,newdata=pd_juv, type="response", se.fit=TRUE)
pd_juv$fit <- preds_juv$fit
pd_juv$se = preds_juv$se.fit

pd <- bind_rows(pd_a, pd_b, pd_juv)

ggplot(df, aes(x = ind_count, y = sharing, color=age, fill=age)) +
  geom_point(position = position_jitter(width = 0.05, height = 0)) +
  geom_ribbon(data = pd, aes(y = fit, ymin = fit - 1.96 * se, ymax = fit + 1.96 * se), 
              alpha = 0.2, linetype=0) +
  geom_line(data = pd, aes(y = fit,color=age)) + 
  theme(panel.background = element_blank())sharomg